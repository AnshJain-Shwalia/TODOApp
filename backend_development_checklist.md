...
- [ ] **Primary Keys**: Prefer **UUIDs** (or time-ordered UUID v7 via `@PrimaryKey({ type: 'uuid' })`) over auto-incrementing integers for client-exposed resources to block resource enumeration attacks.
- [ ] **Foreign Keys & Cardinality in MikroORM**:
    - **1:N**: Place `@ManyToOne(() => Project, { onDelete: 'set null', nullable: true })` on the child entity (e.g. `Todo`).
    - **N:M**: Use `@ManyToMany(() => Tag, tag => tag.todos, { owner: true })` to generate the dedicated join/pivot table with composite primary key `(todo_id, tag_id)`.
- [ ] **Indexing Strategy**:
    - Index all Foreign Key columns (`user_id`, `project_id`) using `@Index()`.
    - Create **Composite Indexes** aligned with query filtering order (e.g., `@Index({ properties: ['user', 'status'] })`).
    - Avoid single-column indexes on low-cardinality fields (e.g., `status` alone).
- [ ] **Soft Delete Implementation**:
    - Include `isDeleted: boolean` (`@Property({ default: false })`) and `deletedAt?: Date` (`@Property({ nullable: true })`).
    - Use MikroORM `@Filter({ name: 'softDelete', cond: { isDeleted: false }, default: true })` or explicit query criteria `WHERE is_deleted = false`.
...
- [ ] **Type-Safe Complex Queries (Kysely + MikroORM)**:
    - Use `em.getKysely({ tableNamingStrategy: 'entity', columnNamingStrategy: 'property' })` for complex SQL joins and analytics while maintaining transaction safety and entity-to-column AST rewriting.
    - _Reference: [mikroorm_and_kysely_mental_model.md](file:///home/ansh/Projects/TODOApp/guides/mikroorm_and_kysely_mental_model.md)_
...
- [ ] **Prevent IDOR & Enforce Tenant Isolation**:
    - Always enforce ownership constraints directly in SQL/MikroORM queries (`em.findOne(Todo, { id, user: userId, isDeleted: false })` or `WHERE id = $1 AND user_id = $2 AND is_deleted = false`).
    - Never query items solely by direct resource ID from URL parameters without checking user identity.
- [ ] **Centralized Auth Middleware / Guards**: Verify JWTs/sessions in an Express middleware or NestJS AuthGuard to inject `req.user` before controllers run.
...
- [ ] **Design Bulk Endpoints**: Use designated batch endpoints (`PATCH /api/v1/todos/bulk`) instead of firing multi-parallel individual HTTP calls.
    - Wrap batch updates in **Atomic Database Transactions** via MikroORM `em.transactional(async (em) => { ... })` when all-or-nothing consistency is required.
